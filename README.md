...
### 2. The "Rush Hour" vs. "Late Night" Cancellation Paradox
While the afternoon commute (17:00) experiences the highest raw volume of cancellations due to sheer traffic density, off-peak hours reveal a more complex operational bottleneck. Specifically, late-night hours like 2:00 AM experience a disproportionately high cancellation rate (**12.7%**). 

This inverse relationship points to a severe supply-side failure: a lack of active drivers during nightlife hours pushes rider wait times past acceptable thresholds. Optimizing driver incentives during these specific late-night windows is a critical opportunity to capture lost revenue.

![Hourly Trip Volume vs. Cancellation Rate](images/hourly_trip_volume_vs._cancellation_rate.png)

> **Methodology Note on 'Total Requests':** The original dataset logs final trip outcomes (completed, driver-cancelled, passenger-cancelled) rather than initial app pings. To calculate the cancellation rate, I engineered a `total_requests` proxy metric: `SUM(COALESCE(completed_trips, 0)) + SUM(COALESCE(driver_cancelled, 0)) + SUM(COALESCE(passenger_cancelled, 0))`. 
>
> While this provides a highly accurate reflection of operational failure rates for matched trips, it is important to note that a "started trip" in the data does not perfectly equal "total passenger demand." It may slightly undercount true demand (e.g., if a user requests a ride but closes the app before the algorithm can match them with a driver).
...
### Missing raw data
**Trip data only**
- 2021-10-07 to 2021-10-09
- 2021-10-14 to 2021-10-16
...
├── sample_data/                        # Lightweight raw CSV subsets (2025 data) for script testing
